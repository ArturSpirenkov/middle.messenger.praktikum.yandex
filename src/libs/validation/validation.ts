import { type ValidationRule, IValidation, ValidationItem, ValidationObject } from './types';
...
export class Validation implements IValidation {
  ...
  constructor (data: {[key: string]: any}, rules: {[key:string]: ValidationRule[]}) {
    this.$rules = rules;
    this.$data = data;
  ...
  __touchValidation (validationField: string) {
    ...
    Object.entries(validation.$rules)
      .forEach(([key, rule]: [key: string, rule: ValidationRule ]) => {
  ...
  $touch () {
    let invalid: boolean = false;
    Object.entries(this.$rules).forEach(([key, rules]: [key: string, rules: ValidationRule[]]) => {
      const validation = this.$validation[key];
      const errors: string[] = [];

      rules.forEach((rule: ValidationRule | Function) => {
        const realRule: ValidationRule = typeof rule === 'function' ? rule() : rule;
...
export const validationRules: { [key:string]: any } = {
  ...
  lengthBoundary: (low: number, up: number): ValidationRule => {
    return {
      $validator: (value: any) => low <= String(value).length && String(value).length <= up,
